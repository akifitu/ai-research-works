...
def summarize_scenario(
    *,
    ...
    mean_recall_at_k = (
        fmean(evaluation.recall_at_k for evaluation in system_evaluations)
...
def summarize_benchmark(scenario_results: list[ScenarioResult]) -> AggregateMetrics:
    ...
    return AggregateMetrics(
        ...
        mean_recall_at_k=round(
            fmean(evaluation.recall_at_k for evaluation in all_evaluations),
            4,
        ),
        mean_mrr=round(fmean(evaluation.mrr for evaluation in all_evaluations), 4),
        mean_coverage=round(
            fmean(evaluation.coverage for evaluation in all_evaluations),
            4,
        ),
        mean_hard_negative_rate=round(
            fmean(evaluation.hard_negative_rate for evaluation in all_evaluations),
            4,
        ),
